...
let llhttp;
try {
  llhttp_simd = require('./llhttp/llhttp_simd.wasm');
  llhttp = require('./llhttp/llhttp.wasm');
} catch (e) {
  llhttp_simd = readFileSync(resolve(__dirname, './llhttp/llhttp_simd.wasm'));
  llhttp = readFileSync(resolve(__dirname, './llhttp/llhttp.wasm'));
}
...
function lazyllhttp () {
  if (!llhttpInstance) {
    let mod
    try {
      mod = new WebAssembly.Module(llhttp_simd)
    } catch (e) {
      ...
      mod = new WebAssembly.Module(llhttp)
    }
